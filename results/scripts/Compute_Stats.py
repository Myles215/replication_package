...
# this causes cropping of the captions - so, https://www.reddit.com/r/learnpython/comments/n09my8/i_have_to_do_a_scientific_poster_and_for_that_i/
...
plt.figure().subplots_adjust(bottom=0.17,left=0.2)

# distributions of method appearances
plt.hist(allMethodsJFR.values(), bins=config['repeats'], range=(1,config['repeats']))
#plt.show()
...
plt.hist(allMethodsHP.values(), bins=config['repeats'], range=(1,config['repeats']))
plt.xlabel("Appearances")
plt.ylabel("Count")
plt.xticks([1,5,10,15,20])
...
plt.figure().subplots_adjust(bottom=0.17,left=0.15)
...
try:
    ...
    print("Number of methods appearing in intersection ", config['typejfr'], " list but not ", config['typehprof'], ":", len((set(intersectionMethodsJFR) - set(intersectionMethodsHP))))
    print("Number of methods appearing in intersection ", config['typehprof'], " list but not ", config['typejfr'], ":", len((set(intersectionMethodsHP) - set(intersectionMethodsJFR))))
    print("Number of methods appearing in both intersection ", config['typejfr'], " list and ", config['typehprof'], ":", len((set(intersectionMethodsHP).intersection(set(intersectionMethodsJFR)))))
...
try:
    ...
    print("nan encountered when plotting histogram")
...
methodsInBothJFRAndHPIntersectionMethods.sort() # sorted to ensure same order every time
plt.scatter(x, y)
plt.xlabel("Average Rank "+config['typejfr'])
plt.ylabel("Average Rank "+config['typehprof'])
...
for method in methodsInBothJFRAndHPIntersectionMethods:
    ...
    plt.hist(testCounts.values(), bins=config['repeats'], range=(1,config['repeats']))
